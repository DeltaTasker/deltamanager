...
export default function ProposalsPage() {
  ...
  const calculateTotals = () => {
    ...
    const tasaISR = concept?.tasaRetencionISR || 0;
    const tasaRetIVA = concept?.tasaRetencionIVA || 0;
    const ivaIncluded = concept?.ivaIncluded || false;
    ...
    if (ivaIncluded) {
      ...
      tax = parseFloat((totalBruto - subtotal).toFixed(2));
      retencionISR = parseFloat((subtotal * (tasaISR / 100)).toFixed(2));
      retencionIVA = parseFloat((tax * (tasaRetIVA / 100)).toFixed(2));
      ...
      tax = parseFloat((subtotal * (tasaIVA / 100)).toFixed(2));
      retencionISR = parseFloat((subtotal * (tasaISR / 100)).toFixed(2));
      retencionIVA = parseFloat((tax * (tasaRetIVA / 100)).toFixed(2));
      total = parseFloat((subtotal + tax - retencionISR - retencionIVA).toFixed(2));
    }

    return { subtotal, tax, retencionISR, retencionIVA, total };
  };
