...
import { getTransactions, type TransactionWithRelations } from "@/modules/transactions/server/queries";
...
export default function IncomePage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [transactionsData, clientsData, conceptsData, bankAccountsData] = await Promise.all([
  ...
  const calculateAmounts = () => {
    ...
    const retencionISR = subtotal * (tasaISR / 100);
  ...
  // Handle edit
  const handleStartEdit = (transaction: TransactionWithRelations) => {
          ...
          <CardContent className="space-y-4">
            ...
            {formData.unitPrice && formData.quantity && (
              <div className="bg-muted p-4 rounded space-y-1">
                ...
                <div>IVA: ${calculateAmounts().iva.toFixed(2)}</div>
                <div>Retención ISR: -${calculateAmounts().retencionISR.toFixed(2)}</div>
                <div className="font-bold">Total: ${calculateAmounts().total.toFixed(2)}</div>
              </div>
            )}
